...
import {
  AddSkillrCredential,
  SkillrCredential,
} from "../../../components/UI/SkillrCredential";
import {
  AddSkillrShowcase,
  SkillrShowcase,
} from "../../../components/UI/SkillrShowcase";
import {
  AddSkillrShowcaseLink,
  SkillrShowcaseLink,
} from "../../../components/UI/SkillrShowcaseLink";
...
const SkillrSkill: React.FC<SkillrSkillProps> = ({ skillrSkill }) => {
  ...
  const [preApprovedRate, setPreApprovedRate] = useState(
    skillrSkill.preApprovedRate
  );
  const [newCredentialName, setNewCredentialName] = useState("");
  const [newCredentialFiles, setNewCredentialFiles] =
    ...
    <>
      ...
      <form onSubmit={handleSubmit}>
        <label>
          ...
          <small>
            Experience and background in training people in{" "}
            {skillrSkill.skill.name}.
          </small>
        ...
        <label>
          <>
            ...
            <ul>
              ...
              <li>
                Available to offer free sessions up to 1 minute per session
              </li>
              <li>
                You will be one of the preferred Skillrs for users to connect
                with
              </li>
            </ul>
          </>
        ...
        <label>
          <>
            Set your own rate
            <p>
              Most {skillrSkill.skill.name} session rates are between{" "}
              <b>$1.00-$5.00 per minute</b>. If you choose to set your own rate
              this Skill will not be ranked as high and will not be applicable
              for learners to try for free.
            </p>
          </>
          <input
            type="radio"
      ...
      <h2>Credentials</h2>
      <p>
        Upload certifications that you have received, this will help us verify
        your credentials.
      </p>
...
export const getServerSideProps: GetServerSideProps<SkillrSkillProps> = async (
  ctx
) => {
  ...
  const skillrSkill = skillrSkills.find(
    (skillrSkill) => skillrSkill.skillrSkillId === skillrSkillId
  );
