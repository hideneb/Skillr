...
const SkillrProfile: React.FC<SkillrProfileProps> = ({
  skillrDDto,
  accountLink,
  stripeLoginLink,
  payoutMethod,
    ...
    <>
      ...
      <div>
        ...
        <div>
          ...
          <button
            type="button"
            onClick={() => Router.push("/skillr/profile/media")}
          >
            Edit
          </button>
        </div>
        <div>
          ...
          <sub>
            Share a photo or video trailer that best showcases who you are
          </sub>
          ...
          <NewMedia
            baseForm={skillrMediaBaseForm}
            accept={skillrMediaAllowedMimeTypes.join(" ")}
          />
        </div>
      </div>
      <div>
        ...
        <ul>
          <li>
            ...
            <span>
              {skillrDDto.localAvailability.day0Begin} -{" "}
              {skillrDDto.localAvailability.day0End}
            </span>
          </li>
          <li>
            <strong>Tuesday</strong>
            <span>
              {skillrDDto.localAvailability.day1Begin} -{" "}
              {skillrDDto.localAvailability.day1End}
            </span>
          </li>
          <li>
            <strong>Wednesday</strong>
            <span>
              {skillrDDto.localAvailability.day2Begin} -{" "}
              {skillrDDto.localAvailability.day2End}
            </span>
          </li>
          <li>
            <strong>Thursday</strong>
            <span>
              {skillrDDto.localAvailability.day3Begin} -{" "}
              {skillrDDto.localAvailability.day3End}
            </span>
          </li>
          <li>
            <strong>Friday</strong>
            <span>
              {skillrDDto.localAvailability.day4Begin} -{" "}
              {skillrDDto.localAvailability.day4End}
            </span>
          </li>
          <li>
            <strong>Saturday</strong>
            <span>
              {skillrDDto.localAvailability.day5Begin} -{" "}
              {skillrDDto.localAvailability.day5End}
            </span>
          </li>
          <li>
            <strong>Sunday</strong>
            <span>
              {skillrDDto.localAvailability.day6Begin} -{" "}
              {skillrDDto.localAvailability.day6End}
            </span>
          </li>
        </ul>
        <button
          type="button"
          onClick={() => Router.push("/skillr/availability")}
        >
          Edit availability
        </button>
      </div>
        ...
        <ul>
          {skillrDDto.languages.map((skillrLanguage) => {
            const language = languages.find(
              (language) => language.id == skillrLanguage.languageId
            );
            if (!language) {
              return null;
            }
...
export const getServerSideProps: GetServerSideProps<SkillrProfileProps> =
  async (context) => {
    ...
    const accountLink = !skillr.stripeAccountId
      ? await createPaymentInfo(token.jwt)
      : null;

    const stripeLoginLink =
      skillr.stripeAccountId &&
      skillr.stripeAccountStatus !== StripeAccountStatus.ENABLED
