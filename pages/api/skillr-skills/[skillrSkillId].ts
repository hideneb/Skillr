...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<SkillrSkillDto>
) {
  const { method, body, query } = req;
  const skillrSkillId = Array.isArray(query.skillrSkillId)
    ? query.skillrSkillId[0]
    : query.skillrSkillId;
  ...
  switch (method) {
    case "PUT":
      const skillrSkillDto = await updateSkillrSkill(
        auth.jwt,
        skillrSkillId,
        body
      );
