...
export const getSkillrBookRateById = async (
  jwt: string,
  skillrBookId: string
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<SkillrBookRateDto>
) {
  ...
  switch (method) {
    case "GET": {
      const skillrBookRate = await getSkillrBookRateById(
        auth.jwt,
        skillrBookId
      );
    ...
    case "POST": {
      const body = req.body;
      const skillrBookRate = await saveSkillrBookRate(
        auth.jwt,
        skillrBookId,
        body
      );
