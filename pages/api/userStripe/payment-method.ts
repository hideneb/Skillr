...
export const getPaymentMethod = async (jwt: string): Promise<Stripe.PaymentMethod> => {
...
export default async function handler(req: NextApiRequest, res: NextApiResponse<Stripe.PaymentMethod>) {
    const { method } = req;
