...
import { findSkills, SkillDto } from "../api/skills";
...
export const getServerSideProps: GetServerSideProps<SkillProps> = async (
  ...
  const skillId = Number(
    Array.isArray(ctx.params?.skillId)
      ? ctx.params?.skillId[0]
      : ctx.params?.skillId
  );
  if (!skillId) {
  ...
  const subSkillSkillrs: { [skillId: number]: SkillrDto[] } =
    await subSkills.reduce(
      async (
        accP: Promise<{ [skillId: number]: SkillrDto[] }>,
        subSkill: SkillDto
      ) => {
