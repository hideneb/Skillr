...
const Card: React.FC<SkillrProfileCardProps> = ({ username, displayName, avatar, about }) => {
		...
		<div className={styles.profileCard}>
			<div className={cx(styles.profileCardContent, styles.card)}>
				<div className={styles.profileCardImageContainer}>
					{avatar ? (
						<Image layout="responsive" height={"100%"} width={"100%"} src={avatar} alt="" />
					) : null}
				</div>
				...
				{!showFullBio && (
					<button className={styles.readMoreButton} onClick={() => setShowFullBio(true)}>
						Read more
					</button>
				)}
				{showFullBio && (
					<button className={styles.readLessButton} onClick={() => setShowFullBio(false)}>
						Read less
					</button>
				)}
			</div>
